<?php
	...
	public function listarVentaControlador($pagina, $registros, $url, $busqueda = "", $fecha_inicio = null, $fecha_fin = null) {
    ...
    $fecha_inicio = $this->limpiarCadena($fecha_inicio);
    ...
    // Campos a seleccionar, incluyendo el cálculo de ganancias
   $campos_tablas = "venta.venta_id, venta.venta_codigo, venta.venta_fecha, venta.venta_hora, venta.venta_total, 
                  ...
                  SUM((venta_detalle.venta_detalle_precio_venta - venta_detalle.venta_detalle_precio_compra) * venta_detalle.venta_detalle_cantidad) as ganancias";

$condiciones = [];

if (!empty($busqueda)) {
    $condiciones[] = "venta.venta_codigo LIKE '%$busqueda%'";
}

if ($fecha_inicio && $fecha_fin) {
    $condiciones[] = "venta.venta_fecha BETWEEN '$fecha_inicio' AND '$fecha_fin'";
}

$where = !empty($condiciones) ? "WHERE " . implode(" AND ", $condiciones) : "";

// Consulta para obtener los datos de las ventas, incluyendo las ganancias
...
$consulta_total_ventas = "SELECT SUM(venta.venta_total) as total_ventas FROM venta $where";

// Consulta para obtener el total de ganancias
$consulta_total_ganancias = "SELECT SUM((venta_detalle.venta_detalle_precio_venta - venta_detalle.venta_detalle_precio_compra) * venta_detalle.venta_detalle_cantidad) as total_ganancias 
                             FROM venta 
                             INNER JOIN venta_detalle ON venta.venta_codigo = venta_detalle.venta_codigo 
                             $where";

// Consulta para contar el número total de registros
$consulta_total = "SELECT COUNT(venta_id) FROM venta $where";
...
$total_ventas = $this->ejecutarConsulta($consulta_total_ventas);
$total_ventas = $total_ventas->fetchColumn();
$total_ventas = number_format($total_ventas, MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR);

$total_ganancias = $this->ejecutarConsulta($consulta_total_ganancias);
$total_ganancias = $total_ganancias->fetchColumn();
$total_ganancias = number_format($total_ganancias, MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR);


    $numeroPaginas = ceil($total / $registros);

    // Mostrar el total de ventas filtradas
    $tabla .= '<div class="notification is-primary">Total de ventas filtradas: ' . MONEDA_SIMBOLO . $total_ventas . ' ' . MONEDA_NOMBRE . '&emsp; &ensp; ////////////////////&emsp; &ensp; Total de Ganancias filtradas: ' . MONEDA_SIMBOLO . $total_ganancias . ' ' . MONEDA_NOMBRE . '</div>'

    ;


 // Obtener los productos más vendidos
    $productos_mas_vendidos = $this->topProductosMasVendidos($fecha_inicio, $fecha_fin);

    // Convertir los resultados en una lista separada por comas
    $lista_productos = [];
    foreach ($productos_mas_vendidos as $producto) {
        $lista_productos[] = $producto['producto_nombre'] . " (" . $producto['total_vendido'] . ")";
    }
    $productos_str = implode(", ", $lista_productos);

    // Mostrar los productos más vendidos
    $tabla .= '<div class="notification is-info">
                <strong>Top 25 Productos más vendidos:</strong><br>' . $productos_str . '
              </div>';
        ...
        <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
            <thead>
                <tr>
                    ...
                    <th class="has-text-centered">Ganancias</th>
    ...
    if ($total >= 1 && $pagina <= $numeroPaginas) {
        ...
        foreach ($datos as $rows) {
            $ganancias = number_format($rows['ganancias'], MONEDA_DECIMALES, MONEDA_SEPARADOR_DECIMAL, MONEDA_SEPARADOR_MILLAR);
            $tabla .= '
                <tr class="has-text-centered">
                    ...
                    <td>' . MONEDA_SIMBOLO . $ganancias . ' ' . MONEDA_NOMBRE . '</td>
                    <td>
...
public function topProductosMasVendidos($fecha_inicio, $fecha_fin) {
    $consulta = "SELECT p.producto_nombre, SUM(vd.venta_detalle_cantidad) as total_vendido
                 FROM venta_detalle vd
                 INNER JOIN producto p ON vd.producto_id = p.producto_id
                 INNER JOIN venta v ON vd.venta_codigo = v.venta_codigo";

    if ($fecha_inicio && $fecha_fin) {
        $consulta .= " WHERE v.venta_fecha BETWEEN '$fecha_inicio' AND '$fecha_fin'";
    }

    $consulta .= " GROUP BY p.producto_nombre
                   ORDER BY total_vendido DESC
                   LIMIT 25";

    return $this->ejecutarConsulta($consulta)->fetchAll();
}
